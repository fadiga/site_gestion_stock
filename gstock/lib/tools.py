...
from vente.form_ import NavigationForm
...
def get_time_pagination(request, year, duration,  duration_number, url,
                                                    additional_args=()):
    ...
    if duration == "week":
        ...
        next_date_url = reverse(url,
                                 args=additional_args + (next_date.year,
                                       duration, next_week_number))

        # formatage de l'affichage des mois
        current_date_ = str(ugettext("Week of")) + " " + \
                        format_date(current_date, ugettext("YYYY MMMM dd"))
        previous_date_ = str(ugettext("Week of")) + " " + \
                        format_date(previous_date, ugettext("YYYY MMMM dd"))
        next_date_ = str(ugettext("Week of")) + " " + format_date(next_date, \
                                                    ugettext("YYYY MMMM dd"))
        ...
        # l'adresse pour afficher le mois précédent
        previous_date_url = reverse(url, args=additional_args +
                                     (previous_date.year, duration,
                                            previous_date.month))

        # l'adresse pour afficher le mois suivant
        next_date_url = reverse(url, args=additional_args + (next_date.year, \
                                       duration, next_date.month))

        # formatage de l'affichage des mois en tenant compte de la
        # language code

        current_date_ = format_date(current_date, "MMMM YYYY")
        previous_date_ = format_date(previous_date, "MMMM YYYY")
        next_date_ = format_date(next_date, "MMMM YYYY")
...
def get_duration_pagination(year, duration, duration_number, url,
                            additional_args=()):
    """
    navigation entre les dates: année, mois, week
    """
    ...
    if duration == "week":
        ...
        week_date_url = reverse(url, args=additional_args + (year, "week", \
                                                        week_numbers[1]))

        # l'adresse pour afficher le mois
        month_date_url = reverse(url, args=additional_args + (year, "month", \
                                                        date.today().month))
